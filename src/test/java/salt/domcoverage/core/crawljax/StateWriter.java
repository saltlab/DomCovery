...
public class StateWriter {
	...
	public void writeHtmlForState(State state, String statefilename) {
		...
		String dom = getDom(statefilename);
	...
	private String getDom(String name) {
		try {
			return FileUtils.readFileToString(new File(ConstantVars.MERGEDLOCATION + name));
		} catch (IOException e) {
	...
	private List<Map<String, String>> getElements(StateFlowGraph sfg, State state) {
		...
		List<Map<String, String>> elements = Lists.newArrayListWithCapacity(candidateElements.size());
